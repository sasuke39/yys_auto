...
class ScriptTask(GameUi, GeneralBattle, DemonEncounterAssets, SwitchSoul):
    ...
    def run(self):
        ...
        if self.config.demon_encounter.switch_soul_by_ch.enable:
            self.ui_get_current_page()
            self.ui_goto(page_shikigami_records)
            switch_soul_by_ch = self.config.demon_encounter.switch_soul_by_ch
            soul_dict = {'1': switch_soul_by_ch.SONGSTRESS,'2': switch_soul_by_ch.SHINKIRO,'3': switch_soul_by_ch.TSUCHIGUMO,
                         '4': switch_soul_by_ch.ODOKURO, '5': switch_soul_by_ch.NAMAZU,
                         '6': switch_soul_by_ch.OBOROGURUMA, '7': switch_soul_by_ch.OBOROGURUMA}
            today_wed = time.strftime("%w", time.localtime())
            self.run_switch_soul(soul_dict[today_wed])
