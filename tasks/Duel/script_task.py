...
class ScriptTask(GameUi, GeneralBattle, DuelAssets, SwitchSoul):
    def run(self):
        con = self.config.duel.duel_config
        # 切换预设队伍御魂
        # if self.config.duel.switch_soul.enable:
        #     self.ui_get_current_page()
        #     self.ui_goto(page_shikigami_records)
        #     self.run_switch_soul(self.config.duel.switch_soul.switch_group_team)
    ...
    def check_score(self, target: int) -> int or None:
        ...
        while 1:
            ...
            new_cr_sc = self.NEW_SCORE.ocr(self.device.image)
            # ocr 数字不合法 第二次拍摄检测
            if current_score < 1200 or current_score > 3000:
                if new_cr_sc < 1200 or new_cr_sc > 3000:
                    continue
                else:
                    check_num = new_cr_sc
            else:
                check_num = current_score
            return check_num if check_num <= target else None
