...
class GeneralBattle(GeneralBuff, GeneralBattleAssets):
    ...
    def switch_preset_team(self, enable: bool=False, preset_group: int=1, preset_team: int=1):
        ...
        logger.info("Try to switch to preset team")
        # 点击预设按钮 查看是否出现2种情况 1有出战按钮 2必须五人的文字 有就下一步
        while 1:
            self.screenshot()
            ...
            if self.appear_then_click(self.I_PRESET, threshold=0.8, interval=1):
                continue
            if self.appear_then_click(self.I_PRESET_NEW, threshold=0.8, interval=1):
                continue
        ...
        while 1:
            ...
            if not self.appear(self.I_PRESET_ENSURE):
                # 尝试点击准备按钮避免卡在预设界面
                self.appear_then_click(self.I_PREPARE_HIGHLIGHT)
                break
